-- GPhil_renderStems_queue_gfx_direct.lua
-- Batch queue STEMS tempo variants with immediate GFX region grid.
...
local function build_queue_summary(rows, tempo_multiplier)
    ...
    local lines = {
        ...
        string.format(
            "STEMS: %d queue project(s), %d region render(s)",
            #regular_groups,
...
local function prompt_per_region_settings(
    ...
    local title = string.format("STEMS Tempo Grid (%d regions)", #rows)
    local win_w = 1240
    local win_h = 760
    ...
    local function dry_run_queue()
        ...
        local plan = log_queue_plan(rows, tempo_multiplier, "=== DRY RUN: optimized STEMS GFX queue plan ===")
...
main = function()
    ...
    local function restore_track_selection()
        reaper.Main_OnCommand(40297, 0)
        for i = 1, #prev_sel do
    ...
    reaper.ShowConsoleMsg("")
    log("=== GPHIL STEMS GFX - queue with CLICK ===")
    log("Stems selection: current selected tracks; preset handles selected-track rendering.")
    ...
    log(string.format("STEMS pattern base: %s", STEMS_PATTERN_BASE))
    ...
    log("Base tempo handling: each region row uses its own detected base tempo.")
    log("--------------------------------------")
    ...
    if per_region_mode then
        ...
        if not command_is_configured(CMD_APPLY_GPHIL_STEMS) then
            log("GPHIL_STEMS preset action not configured. Skipping STEMS renders.")
        else
            for group_index = 1, #regular_groups do
                ...
                if not apply_preset(CMD_APPLY_GPHIL_STEMS, "GPHIL_STEMS") then
                    log("Stopping: failed to apply GPHIL_STEMS preset.")
                    stopped_early = true
                    break
                end
                restore_track_selection()

                local new_pattern = stems_pattern_for_tempo(group.tempo_display)
                ...
                log_existing_output_result(
                    string.format("STEMS group %d", group_index),
                    existing,
                    unchecked,
                    queue_options.skip_existing
                )
                ...
                if #active_regions == 0 then
                    log(string.format("STEMS group %d SKIP: all likely outputs already exist.", group_index))
                else
                    local delete_ok = delete_existing_outputs(
                        string.format("STEMS group %d", group_index),
                        existing,
                    ...
                    if not delete_ok then
                        log(string.format(
                            "STEMS group %d SKIP: one or more existing outputs could not be deleted.",
                            group_index
                        ))
                    else
                        select_regions(proj, active_regions)
                        ...
                        log(string.format(
                            "STEMS group %d QUEUED: %s (%s), tempo %g (base %g, factor %.6f) -> %s",
                            group_index,
                            region_rows_label(active_regions),
                            region_count_label(#active_regions),
                            group.target_tempo,
        ...
        log("--------------------------------------")
        log(string.format(
            "Per-region STEMS queue %s. Click entries: %d (%d regions), STEMS entries: %d (%d regions), skipped existing region outputs: %d.",
        ...
        -- 2) STEMS renders with GPHIL_STEMS
        if not command_is_configured(CMD_APPLY_GPHIL_STEMS) then
            log("GPHIL_STEMS preset action not configured. Skipping STEMS renders.")
        else
            ...
            while (step > 0 and tempo <= end_tempo) or (step < 0 and tempo >= end_tempo) do
                ...
                local new_pattern = stems_pattern_for_tempo(tostring(tempo_display))
                set_render_pattern(proj, new_pattern)
    ...
    reaper.Undo_EndBlock("GPHIL STEMS GFX - queue tempo variants", -1)

    if not ok then
        log("--------------------------------------")
        log("ERROR: " .. tostring(err))
        reaper.ShowMessageBox(
            "GPHIL STEMS GFX stopped after an error. Original project state was restored.\n\n" .. tostring(err),
            "Error",
            0
        )
        return
    ...
    log("Original tempo map, render settings, CLICKTRACK/SOLO mute, stem mute states, selection, and region selection restored.")
end
